...
static struct ouichefs_superblock *write_superblock(int fd, struct stat *fstats)
{
	...
	nr_bstore_blocks = idiv_ceil(nr_data_blocks * sizeof(struct ouichefs_block_info), OUICHEFS_BLOCK_SIZE);
	nr_data_blocks -= nr_bstore_blocks;
	...
	sb->nr_bstore_blocks = htole32(nr_bstore_blocks);
	...
	printf("Superblock: (%ld)\n"
	       ...
	       sb->nr_free_blocks, sb->nr_bstore_blocks);
...
static int write_bfree_blocks(int fd, struct ouichefs_superblock *sb)
{
	...
	/*
	 * First blocks (incl. sb + istore + ifree + bfree + bstore + 1 used block)
...
static int write_bstore_blocks(int fd, struct ouichefs_superblock *sb)
{
	int ret = 0;
	uint32_t i;
	char *block;
	...
	/* Block infos store */
	memset(bfree, 0, OUICHEFS_BLOCK_SIZE);
	for(i = 0; i < le32toh(sb->nr_bstore_blocks); i++) {
		ret = write(fd, bfree, OUICHEFS_BLOCK_SIZE);
		if (ret != OUICHEFS_BLOCK_SIZE) {
			ret = -1;
			goto end;
	...
	printf("Bstore blocks: wrote %d blocks\n", i);
...
int main(int argc, char **argv)
{
	...
	/* Write block store blocks */
	ret = write_bstore_blocks(fd, sb);
	if (ret != 0) {
		perror("write_bstore_blocks()");
		ret = EXIT_FAILURE;
		goto free_sb;
	}
