...
static void ouichefs_put_super(struct super_block *sb)
{
	...
	kfree(sbi->isrc_bitmap);
	kfree(sbi->idup_bitmap);
	kfree(sbi);
}
...
static int ouichefs_sync_fs(struct super_block *sb, int wait)
{
	...
	disk_sb->nr_bstore_blocks = sbi->nr_bstore_blocks;
	disk_sb->index_dupblock   = sbi->index_dupblock; 
	...
	/* Flush block info store */
	for(i = 0; i < sbi->nr_bstore_blocks; i++){
		int idx = OUICHEFS_FIRST_BINFO_STORE(sbi) + i;
...
int ouichefs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	sbi->nr_bstore_blocks = csb->nr_bstore_blocks;
	sbi->index_dupblock = csb->index_dupblock;
	sb->s_fs_info = sbi;
	...
	/* Alloc isrc_bitmap - freed in put_super() */
	sbi->isrc_bitmap = kzalloc(sbi->nr_ifree_blocks * OUICHEFS_BLOCK_SIZE, GFP_KERNEL);
	if (!sbi->isrc_bitmap) {
		ret = -ENOMEM;
		goto free_bfree;
	}
	
	/* Alloc idup_bitmap - freed in put_super() */
	sbi->idup_bitmap = kzalloc(sbi->nr_ifree_blocks * OUICHEFS_BLOCK_SIZE, GFP_KERNEL);
	if (!sbi->idup_bitmap) {
		ret = -ENOMEM;
		goto free_isrc;
	}
	...
	/* Alloc index_dupbloc, only on first fileSystem mount */ 
	if(sbi->index_dupblock == 0){	
		/* Get a free block for dup_block & add it to sb_info */
		uint32_t bno = get_free_block(sbi);
		if (!bno) {
			ret = -ENOSPC;
			goto iput;
		}
		sbi->index_dupblock = bno;
	...
	/* Deduplicate blocks */
	ret = ouichefs_deduplicate(sb);
	if(ret < 0)
		pr_warn("Error: deduplication failed !!");
	
	return 0;
bput:	
	put_block(sbi, sbi->index_dupblock);
iput:
	iput(root_inode);
free_idup:
	kfree(sbi->idup_bitmap);
